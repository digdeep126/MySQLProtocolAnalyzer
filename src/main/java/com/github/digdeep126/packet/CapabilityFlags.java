...
public enum CapabilityFlags {
	...
	CLIENT_SSL(0x00000800,"Server Supports SSL. Client Switch to SSL after sending the capability-flags."),	
...
//	Although the OK packet is extensible, the EOF packet is not due to the overlap of its bytes with 
//	the content of the Text Resultset Row.
//
...
//	EOF packets are deprecated as of MySQL 5.7.5.
    ...
    private CapabilityFlags(int code, String desc) {
        this.code = code;
        this.desc = desc;
    }
    public int getCode() {
     ...
     * @return 枚举
     */
    public static final CapabilityFlags getByCode(int code) {
        if (code <0x00000001 || code > 0x01000000) {
            return null;
        }
        for (CapabilityFlags item : CapabilityFlags.values()) {
